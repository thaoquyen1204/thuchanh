...
CREATE LOGIN TruongNhom WITH PASSWORD = '12345678';
GO
...
USE AdventureWorks2008R2;
GRANT SELECT,INSERT, UPDATE,DELETE ON Production.ProductInventory TO TruongNhom;
GO

-- Phân quyền cho nhân viên NV
USE AdventureWorks2008R2;
GRANT SELECT,INSERT,UPDATE, DELETE ON Production.ProductInventory TO NhanVien;
GO
...
--Chỉ có trưởng nhóm TN và nhân viên có thể sửa được dữ liệu bảng Production.Product, vì họ được phân quyền SELECT,INSERT, UPDATE và DELETE trên bảng này.

--e. Thu hồi quyền cấp cho nhân viên NV:

-- Thu hồi quyền của nhân viên NV
USE AdventureWorks2008R2;
REVOKE SELECT,INSERT,UPDATE, DELETE ON Production.ProductInventory FROM NHANVIEN;
GO
