...
SELECT m.MaVT, m.TenVT, sum(n.SoLuongN-x.SoLuongX+m.SoLuongT) as "SL còn lại"
FROM Ton m
INNER JOIN Nhap n ON m.MaVT = n.MaVT
INNER JOIN Xuat x ON m.MaVT = x.MaVT
where year(n.NgayN) = 2015 OR year(x.NgayX) = 2015
GROUP BY m.MaVT,m.TenVT;
go
SELECT * FROM CAU10
